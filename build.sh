...
M2VOLUME="$(echo ~)/.m2"
VOLUMES="-v ${TEMPVOLUME}:/workspace -v ${M2VOLUME}:/root/.m2 -v /var/run/docker.sock:/var/run/docker.sock -v ${DOCKER_BINPATH}:/bin/docker"
...
docker run --rm ${VOLUMES} -w /workspace bravo/xmvn:3.3.9 ${MVN_ARGS}
